function [dEH] = MaxwellENOrhs1D(x, EMf, ep, mu, h, k,m, Crec, maxvel)
% function [dEH] = MaxwellENOrhs1D(x, EM, ep, mu, h, k,m, Crec, maxvel)
...
for i = 1:N+2
    [Em(i), Ep(i) ] = ENO(xe(1:(i+2*(m-1))), Ee(i:(i+2*(m-1))), m , Crec);
    [Hm(i), Hp(i) ] = ENO(xe(1:(i+2*(m-1))), He(i:(i+2*(m-1))), m , Crec);
end
