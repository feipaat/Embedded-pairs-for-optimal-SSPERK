...
erk = EmbeddedRK('RungeKutta',rk, 'RelativeTolerance',1e-3,...
    'AbsoluteTolerance',1e-3,'Name','Merson45', 'Controller',controller);
...
elseif strcmpi(problem, 'advection')
    ...
    % Solver Problem
    [u] = Scalar1D(x, Q, h, m, CFL, FinalTime, flux, maxvel, UEXACT);
    
elseif strcmpi(problem, 'vdp')
    FinalTime = 2.0;
    odeObj = initODE(problem);
    
    [u] = SimpleODE1D(odeObj.y0, odeObj, FinalTime, UEXACT);
